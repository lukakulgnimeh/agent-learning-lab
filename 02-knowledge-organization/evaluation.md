...
- Overall structure is good, follows given format, but individual answers differ in appearence. Quality is enough for a quick refresher and allows follow up questions.
...
- Extracts "randomly" if graph or diagramm is present such that no real structure noticeable.
...
- Respect chunk sizes of data loader to avoid further subsplitting.
...
- Sometimes does not asked retriever ageain and started halluzinating own answers without references. Although it was supposed to answer with a specific answer in that case.
